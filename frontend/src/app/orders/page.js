      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="p-6 rounded-lg border text-center" style={{ background: 'var(--surface)', borderColor: 'var(--error)' }}>
            <p className="text-red-500 font-medium mb-4">{t('orders.errorLoading')}</p>
                  ...
                  >
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-3">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          >
                            {isExpanded ? t('orders.hideDetails') : t('orders.viewDetails')}
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="px-6 pb-6 pt-2 border-t" style={{ borderColor: 'var(--border)' }}>
                        <h4 className="font-medium mb-3 mt-3" style={{ color: 'var(--foreground)' }}>{t('orders.items')}</h4>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                          <div>
                            <h4 className="font-medium mb-1" style={{ color: 'var(--foreground)' }}>{t('checkout.shippingAddress')}</h4>
                            <p className="text-sm" style={{ color: 'var(--muted)' }}>
                          ...
                          <div>
                            <h4 className="font-medium mb-1" style={{ color: 'var(--foreground)' }}>{t('orders.payment')}</h4>
                            <p className="text-sm" style={{ color: 'var(--muted)' }}>
                              {t('orders.method')} {order.payment?.method || '—'}<br />
                              {t('orders.status')} {paymentStatusLabel}
                            </p>
                          </div>
                        </div>
                        ...
                        {order.tracking?.trackingNumber && (
                          <div className="mt-4 pt-3 border-t text-sm" style={{ borderColor: 'var(--border)' }}>
                            <span style={{ color: 'var(--muted)' }}>{t('orders.tracking', { carrier: order.tracking.carrier })} </span>
            ...
            {pagination.pages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-8">
                ...
                <span className="text-sm" style={{ color: 'var(--muted)' }}>
                  {t('orders.pageOf', { page: pagination.page, pages: pagination.pages })}
                </span>
                <button
                  onClick={() => fetchOrders(pagination.page + 1)}
